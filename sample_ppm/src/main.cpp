...
{
	errno_t error;
	error = freopen_s(&stream, "sample.txt", "w", stdout);
	if( error != 0 )
	...
	fprintf(stdout, "current dir is : %s", getCurrentWorkingDir().c_str());

    auto w = 1280;      // 画像の横幅.	//width
    auto h = 1080;      // 画像の縦幅.	//height
